...
$(document).ready(function () {
    ...
    // Initialize fields if values exist
    if (initialSchoolId && initialDepartmentId) {
        $.ajax({
            url: getDepartmentsUrl,
            data: { 'school_id': initialSchoolId },
            success: function (data) {
                $('#id_department').empty();
                $.each(data, function (key, value) {
                    $('#id_department').append($('<option></option>').attr('value', value.id).text(value.name));
                });
                $('#id_department').val(initialDepartmentId).trigger('change');
            }
        });
    }

    if (initialStateId && initialLgaId) {
        $.ajax({
            url: getLgasUrl,
            data: { 'state_id': initialStateId },
            success: function (data) {
                $('#id_local_government').empty();
                $.each(data, function (key, value) {
                    $('#id_local_government').append($('<option></option>').attr('value', value.id).text(value.name));
                });
                $('#id_local_government').val(initialLgaId).trigger('change');
            }
        });
    }
});
